...
# Função para limpar instalações anteriores de NVIDIA
cleanup_old_nvidia() {
    msg "Limpando instalações anteriores de NVIDIA..."
    
    # Tentar remover quaisquer drivers NVIDIA antigos
    echo -e "${BLUE}Removendo pacotes NVIDIA conflitantes...${NC}"
    
    # Lista de pacotes NVIDIA que podem causar conflito
    local nvidia_conflict_packages=(
        nvidia-525xx-dkms nvidia-525xx-utils
    ...
    echo -e "${GREEN}✅ Limpeza de drivers antigos concluída${NC}"
}
...
progress_install() {
    ...
    # Para pacotes NVIDIA, usar instalação especial
    if [[ "$description" == *"NVIDIA"* ]]; then
        echo -e "${YELLOW}⚠️  Instalação especial para drivers NVIDIA...${NC}"
        
        # Instalar cada pacote NVIDIA individualmente
        for pkg in "${to_install[@]}"; do
            echo -e "${BLUE}Instalando $pkg...${NC}"
            sudo pacman -S --needed --noconfirm "$pkg" || {
                echo -e "${YELLOW}Aviso: $pkg pode ter problemas de dependência${NC}"
        ...
        if command -v yay &> /dev/null; then
            echo -e "${BLUE}Usando yay para instalação...${NC}"
            yay -S --needed --noconfirm "${to_install[@]}" || {
                echo -e "${YELLOW}Tentando com pacman...${NC}"
                sudo pacman -S --needed --noconfirm "${to_install[@]}" || exit_code=1
            }
        else
            sudo pacman -S --needed --noconfirm "${to_install[@]}" || exit_code=1
        fi
...
# Função para configurar drivers NVIDIA
configure_nvidia() {
...
EOF
    ...
    # Adicionar configuração de módulo NVIDIA
    if [ ! -f /etc/modprobe.d/nvidia.conf ]; then
        echo -e "${BLUE}Criando configuração para driver NVIDIA...${NC}"
...
EOF
    ...
    # Configurar módulos para carregar na inicialização
    if [ -f /etc/mkinitcpio.conf ]; then
        if ! grep -q "nvidia nvidia_modeset nvidia_uvm nvidia_drm" /etc/mkinitcpio.conf; then
            echo -e "${BLUE}Configurando módulos NVIDIA no mkinitcpio...${NC}"
    ...
    echo -e "${BLUE}Verificando instalação do driver NVIDIA...${NC}"
    if command -v nvidia-smi &> /dev/null; then
        if nvidia-smi &> /dev/null; then
            echo -e "${GREEN}✅ Driver NVIDIA funcionando corretamente${NC}"
        else
            echo -e "${YELLOW}⚠️  Driver NVIDIA instalado mas não funcionando${NC}"
            echo -e "${YELLOW}Pode ser necessário reiniciar o sistema${NC}"
        fi
    else
...
if [ "$ONLY_CONFIG" = false ]; then
    # Limpar drivers antigos antes de instalar
    cleanup_old_nvidia
    ...
    # Instalar NVIDIA de forma especial
    progress_install "Installing NVIDIA drivers" "${PACKAGES_NVIDIA[@]}" || {
        echo -e "${YELLOW}⚠️  Tentando instalação alternativa dos drivers NVIDIA...${NC}"
        # Tentar instalar manualmente se a função progress_install falhar
        for pkg in "${PACKAGES_NVIDIA[@]}"; do
            echo -e "${BLUE}Instalando $pkg manualmente...${NC}"
            sudo pacman -S --needed --noconfirm "$pkg" || {
